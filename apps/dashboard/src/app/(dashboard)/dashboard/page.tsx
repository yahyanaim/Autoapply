...
export default function DashboardPage() {
  ...
  const usage = useQuery<UsageSummary>({
    queryKey: ['ai-usage'],
    queryFn: () => apiClient.get('/ai/usage'),
  });
  const items = applications.data?.applications ?? [];
  const total = applications.data?.total ?? 0;
  const interviews = items.filter(
    (item) => item.status === 'interview' || item.status === 'offer',
  ).length;
  const interviewRate = total
    ? Math.round((interviews / Math.min(total, items.length || total)) * 100)
    : 0;
  const quota = usage.data?.quota ?? null;

  const stats = [
    {
      label: 'Total applications',
      value: String(total),
      change: 'Tracked across all stages',
      trend: 'neutral' as const,
    },
    {
      label: 'Resumes',
      value: String(resumes.data?.length ?? 0),
      change: `${resumes.data?.filter((resume) => resume.parsedJson).length ?? 0} ready`,
      trend: 'neutral' as const,
    },
    {
      label: 'Interview rate',
      value: `${interviewRate}%`,
      change: 'From recent applications',
      trend: interviewRate > 0 ? ('up' as const) : ('neutral' as const),
    },
  ];
