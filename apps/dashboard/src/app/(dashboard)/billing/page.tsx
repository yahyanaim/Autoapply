...
import { pricingPlans, PublicPricingPlan } from '@/lib/pricing';
import { PaidPlan, readRequestedPaidPlan } from '@/lib/post-auth-plan';
...
export default function BillingPage() {
  ...
  const checkout = useMutation({
    mutationFn: (plan: 'pro' | 'premium') =>
      apiClient.post<{ url: string }>('/billing/checkout-session', { plan }),
  });
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Plans and invoices are securely managed by Stripe.
        </p>
      </div>
      ...
      {requestedPlan && subscription.data?.plan !== requestedPlan && (
        <div className="rounded-lg border border-info-200 bg-info-50 p-3 text-sm text-info-700">
          You selected the{' '}
          <span className="font-semibold capitalize">{requestedPlan}</span>{' '}
          plan. Review its included limits below, then continue securely with
          Stripe.
        </div>
      )}
