...
const api = vi.hoisted(() => ({ jobs: vi.fn(), resumeFailures: vi.fn(), betaGate: vi.fn(), notifications: vi.fn(), applications: vi.fn() }));
...
describe('Phase 3 Admin operational pages', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    api.jobs.mockResolvedValue({ jobs: [{ id: 'job-1', title: 'Engineer', company: 'ApplyAI', source: 'greenhouse', location: 'Remote', remoteType: 'remote', lastObservedAt: '2026-09-24T00:00:00.000Z', eligible: true, createdAt: '2026-09-24T00:00:00.000Z' }], limit: 20, nextCursor: null });
    api.resumeFailures.mockResolvedValue({ failures: [{ resumeId: 'resume-1', status: 'failed', failureCategory: 'processing_failed', mimeType: 'application/pdf', executionCount: 2, lastAttempt: 2, lastAttemptAt: '2026-09-24T00:00:00.000Z', createdAt: '2026-09-23T00:00:00.000Z', failedAt: '2026-09-24T00:00:00.000Z' }], limit: 20, nextCursor: null });
  ...
  it('renders jobs and resume failures without mutation controls or sensitive content', async () => {
    ...
    expect(resumes.container.textContent).not.toMatch(/resume text|parsedJson|originalFileUrl|prompt|token|provider|requeue button/i);
    resumes.cleanup();
  });
  ...
  it('shows loading and safe error retry states without calling mutation APIs', async () => {
    ...
    expect(Object.keys(api).sort()).toEqual(['applications', 'betaGate', 'jobs', 'notifications', 'resumeFailures']);
  });
