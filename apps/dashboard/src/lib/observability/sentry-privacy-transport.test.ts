...
describe('createPrivacySafeTransport', () => {
  it('sends only a sanitized error event and drops browser envelope data', async () => {
    const sent: unknown[] = [];
    const transport = {
    ...
    await privacySafeTransport.send([
      ...
      [
        ...
        [{ type: 'attachment', filename: 'john_smith.resume' }, 'FULL_CV_CONTENT'],
              ...
              values: [
                {
                  ...
                  stacktrace: {
                    frames: [
                      {
                        filename: '/app/resume-page.tsx?token=ACCESS_TOKEN',
                        function: 'saveResume',
                        lineno: 8,
                        colno: 4,
                      },
  ...
  it('does not forward an envelope without an eligible error event', async () => {
    const send = vi.fn().mockResolvedValue({ status: 'success' });
    const transport = { send, flush: () => Promise.resolve(true) };
    const privacySafeTransport = createPrivacySafeTransport(
      transport,
      sanitizeSentryEvent,
    );
