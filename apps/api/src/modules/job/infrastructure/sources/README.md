...
greenhouse/ (Job Board API), lever/ (Postings API), ashby/ (public Job
Postings API at `api.ashbyhq.com`).
Do not add a LinkedIn/Indeed adapter here -- those require DOM scraping,
which is out of scope without separate legal review.

CV discovery may refresh a bounded set of these JSON API sources through
