...
Optimize the candidate's resume to maximize alignment with the target job description while maintaining complete honesty. The output must read as if the candidate wrote it — not as if an AI generated it.
...
1. **Keyword injection through rephrasing.** Identify high-value keywords from the JD that the candidate already possesses but hasn't explicitly mentioned. Weave them into existing bullet points naturally.
...
| Summary/Objective | Rewrite to directly address the role; include top 3 JD keywords |
...
```json
{
  "optimizedResumeText": "<full optimized resume as plain text>",
  "changesSummary": [
    {
...
The `fabricationCheck` field must always be `"passed"`. If you detect that you cannot optimize without fabricating, return the original text unchanged and explain in `changesSummary`.
