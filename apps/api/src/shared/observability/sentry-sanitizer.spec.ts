      ...
      values: [
        {
          ...
          stacktrace: {
            frames: [
              {
                filename:
                  '/Users/john_smith.resume/parser.ts?token=ACCESS_TOKEN',
                function: 'Jane.Doe',
                lineno: 42,
                colno: 7,
                in_app: true,
...
describe('createSentryEventSanitizer', () => {
  ...
  it('creates a new minimal event and never copies sensitive candidate data', () => {
        ...
        values: [
          {
            type: 'ApplyAIError',
            stacktrace: {
              frames: [{ lineno: 42, colno: 7, in_app: true }],
            },
          },
        ],
      },
  ...
  it.each([
    ...
    ['a huge array', () => ({
      ...
      breadcrumbs: Array.from(
        { length: SENTRY_PRIVACY_LIMITS.maxArrayLength + 1 },
        () => 'safe',
      ),
    })],
    ['too many exception values', () => ({
      ...
      exception: {
        values: Array.from(
          { length: SENTRY_PRIVACY_LIMITS.maxExceptionValues + 1 },
          () => ({ type: 'Error' }),
        ),
      },
    })],
        ...
        values: [
          {
            stacktrace: {
              frames: Array.from(
                { length: SENTRY_PRIVACY_LIMITS.maxStackFrames + 1 },
                () => ({}),
              ),
            },
          },
    ...
    ['an oversized source event', () => ({
      ...
      extra: Object.fromEntries(
        Array.from({ length: 40 }, (_, index) => [
          `safe${index}`,
          'x'.repeat(SENTRY_PRIVACY_LIMITS.maxStringLength),
        ]),
      ),
    })],
    ['an Error cause chain', () => ({
      ...eventWithSensitiveData(),
      cause: new Error('FULL_CV_CONTENT', {
        cause: new Error('PROMPT_TEXT'),
      }),
    })],
  ...
  it('rejects malformed events and invalid trusted configuration', () => {
    ...
    ).toThrow('Sentry environment must be an approved deployment name');
  });
});
